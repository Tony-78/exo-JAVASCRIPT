...
let countSpaces = function (sentence) {
  ...
  // split : couper la chaine de caractère en tableau
}

//Exercice 12 : Inverser une chaîne de caractères.
let invertSentence = function (sentence) {
  return sentence.split('').reverse().join('');
  // split('') on coupe à chaque caractère
  // join remettre en string
}
...
let calculateArea = function (radius) {
  let answer = Math.round(Math.PI * radius * radius);
  return answer;
